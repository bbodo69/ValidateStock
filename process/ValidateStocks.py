...
for idx, row in df.iterrows():
    ...
    for i in dicMVPatten.keys():
        # 날짜, 매수날종가, 소요기간, 구분 리턴
        dicBuySellResult = resultBuySell.GetBuySellResultUseDatePriceExpireDate(df=dfCode, date=i, buyRate=0.99,
                                                                                sellRate=1.025, exDate=50)
        dicScatterDate[i] = {}
    ...
    Image.SaveDFImageWithScatter2(df=dfCode, x='날짜', y='종가', dicScatterData=dicScatterDate, title=row['code'],
                                  savePath=imgFilePath)

    dfTotal.loc[len(dfTotal)] = {'종목코드': row['code'], 'totalCnt': totalCnt, 'totalCnt0': totalCnt0,
                                 'totalCnt1': totalCnt1, 'totalCnt2': totalCnt2, 'period': avgPeriod,
                                 'avgPeriod': avgSellPeriod}

    excel_collection.saveDFToNewExcel(resultFilePath, 'result', dfTotal)

    print('{0} / {1} ::: totalCnt : {2}, totalCnt0 : {3}, totalCnt1 : {4}, totalCnt2 : {5}'.format(idx + 1, len(df),
                                                                                                   totalCnt, totalCnt0,
                                                                                                   totalCnt1,
                                                                                                   totalCnt2))
